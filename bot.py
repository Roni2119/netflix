...
CHANNELS = ['@roni_here', '@black_mrket'] # Dono channels check honge
ADMIN_ID = 5097298890 # Aapki verified ID
REQUIRED_REFERS = 3
...
def get_join_markup():
    ...
    markup.add(types.InlineKeyboardButton("🔄 I Have Joined All", callback_data="check_join"))
    return markup
...
def callback_check(call):
    if is_member(call.from_user.id):
        ...
        bot.answer_callback_query(call.id, "❌ Saare channels join nahi kiye!", show_alert=True)
...
def claim(message):
    ...
    if user and user[1] >= REQUIRED_REFERS:
        if not os.path.exists("accounts.txt"):
            safe_send(user_id, "❌ Error: accounts.txt nahi mili.")
            return
        ...
        safe_send(user_id, f"🎁 **Aapka Netflix Account:**\n\n`{gift}`")
        conn = sqlite3.connect('bot_data.db')
...
def manual_points(message):
    if message.from_user.id == ADMIN_ID:
        try:
            ...
            conn.commit()
            conn.close()
            safe_send(ADMIN_ID, f"✅ User `{uid}` ko {pts} points diye gaye.")
            safe_send(uid, f"🎁 Admin ne aapko {pts} points bonus diye hain!")
        except: safe_send(ADMIN_ID, "Use: `/addpoint 12345 5`")
...
def broadcast_msg(message):
    if message.from_user.id == ADMIN_ID:
        ...
        safe_send(ADMIN_ID, "✅ Broadcast Done!")
